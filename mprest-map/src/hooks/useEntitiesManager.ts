...
export const useEntitiesManager = () => {
    ...
    const findEntity = useCallback((entityId: string, layerId?: string) => {
    ...
    const selectEntity = useCallback((entityId: string, layerId?: string, flyTo?: boolean | number) => {
        ...
        console.warn(`Entity with id "${entityId}" not found${layerId ? ` in layer "${layerId}"` : ''}`);
        return false;
